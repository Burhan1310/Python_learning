...
class task:
    ...
    def __init__(self, title, description, due_date):  # to construct a object of task
        self.title = title
    ...
    def mark_as_completed(self):  # method to mark task as completed
        self.is_complete = "Yooh! Completed"
...
class taskManager:
    def __init__(self):
        self.tasks = []  # constructor that initialize a empty list
...
def menu(task_manager):
    ...
    if choice == 1:
        os.system("cls")
        task_manager.add_task(task(title=input("Enter title : "),description=input("Enter Description : ")))
